...
function SolveModel(inputData::NamedTuple, vars::NamedTuple, params::NamedTuple, κ̂ʲ::Array{Float64,3};
    newData=false, damp=0.8, updateData=1, tol=1e-6, maxIter=1e3, power=false,
    ...
    function UpdateRule(X)
        ...
        τʲ′ = τʲ .* τ̂ʲ
        πʲ′ = πʲ .* (τ̂ʲ .* reshape(ĉʲ, 1, size(ĉʲ)...) ./ reshape(p̂ʲ', N, J, 1)) .^ reshape(θʲ, 1, J, 1)
        Inˡ′ = Inˡ .* ŵ₀
        Inᶠ′ = Inᶠ .* p̂ᶠ₀ .^ (1 .+ ηᶠ)

        # solve Xʲ, Yʲ when we have p̂ʲ, ĉ
        Xʲ′, Yʲ′, In′ = OutputFunc(ξʲ, ξʲᵏ, αʲ, πʲ′, τʲ′, Inˡ′, Inᶠ′, D, J, N)
...
#
function OutputFunc(ξʲ, ξʲᵏ, αʲ, πʲ′, τʲ′, Inˡ′, Inᶠ′, D, J, N;
    tol = 1e-8, 
    maxIter = 1e3,
    damp = 0.0,
    power = false,
    ...
    function UpdateRule(Xʲ′₀)
        Yʲ′ = sumsqueeze(repeat(Xʲ′₀', 1, 1, N) .* πʲ′ ./ (1 .+ τʲ′), dims = 1);
        Inᵗ′ = vec(sum(repeat(Xʲ′₀', 1, 1, N) .* πʲ′ ./ (1 .+ τʲ′) .* τʲ′, dims = (2, 3)));
